...
void checkCollision(Power power){
  ...
  // --- Check collision (box) ---
  bool hitX = (projX >= hitBoxLeft && projX <= hitBoxRight);
  bool hitY = (projY >= hitBoxTop  && projY <= hitBoxBottom);

  Serial.printf("proj size: %d\n");
  Serial.printf("proj(%.1f, %.1f) | tank: (%.1f, %.1f)\n", projX, projY, targetX, floorHeight - tankSizeY);
